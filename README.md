...
Brain Capture 是 BrainHub 的 Chrome Manifest V3 采集扩展。它按站点授权访问当前已登录的网页会话，将 ChatGPT、Claude、Gemini 和 Grok 的可见活动分支转换为统一 Markdown，并写入个人 Google Drive 的 `brain-hub/` 目录。
...
3. 先打开并登录目标 AI 站点，再在弹窗中逐站开启；Chrome 只会请求该站点的访问权限。
4. 点击站点旁的同步按钮，或点击顶部“同步全部”。首次同步会进行全量补拉。
5. 站点内部接口发生变化时，弹窗和扩展徽标会显示异常；其他站点仍独立继续同步。
